...
function displayProductList() {
	...
	products.forEach((p, index) => {
		productInfo += createProductItemInfo(p, index)
	});
...
function createProductItemInfo(item, index) {
	...
	trow += `<tr><td>${item.name}</td><td>${item.type}</td><td>${item.price}</td>
			<td><input type='checkbox' onclick="updateCart(${index})" id=c-${index}></td></tr>`;
	return trow;
}
...
function displayResult(){
	const newArray = selectedItems.map(item => {
		if(selectedItems) {
			...
			const taxes = products.reduce(() => subtotal * 0.05);
			
			// Calculate the final amount
			const final_amount = products.reduce(() => (subtotal + taxes)*0.80);
...
function deleteItem(index) {
	alert("You removed an item from your shopping cart!");
	selectedItems = selectedItems.filter(function(item){
		return item !== index;
	});
	displayShoppingCart();
